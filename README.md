...
Make sure `PUBLIC_URL` matches the externally accessible HTTPS address of your PentAGI instance. If the URL configured in the OAuth provider does not exactly match the callback generated by PentAGI, the provider will reject the login attempt with a redirect URI mismatch error.
