...
export const FlowDashboardOverview = ({ flowId }: { flowId: string }) => {
    ...
    const agentTypeRows = (usageByAgentData?.usageStatsByAgentTypeForFlow ?? []).map((item) => ({
        label: item.agentType,
        stats: item.stats,
    }));

    const toolcallsByFunction = [...(toolcallsByFunctionData?.toolcallsStatsByFunctionForFlow ?? [])].sort(
